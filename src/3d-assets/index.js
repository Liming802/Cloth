...
window.addEventListener("click", (event) => {
  ...
  const intersects = raycaster.intersectObjects(cloths.map(({ mesh }) => mesh));
  if (intersects.length > 0) {
    const clickedCloth = intersects[0].object;
    ...
    infoBox.innerHTML = `Someone's fashionable coat, worn once, has 1.3% chance of being recycled: ${clickedCloth.material.color.getStyle()}</p>`;
    ...
    // Remove info box when clicking outside
    const removeInfoBox = (e) => {
      if (e.target !== infoBox) {
        infoBox.remove();
        window.removeEventListener("click", removeInfoBox);
      }
    };
    window.addEventListener("click", removeInfoBox, { once: true });
  }
});
...
import { captureAndIdentifyChestColor } from './camera.js';
...
window.addEventListener("keydown", (event) => {
  if (event.code === "Space") {
    const identifiedColor = captureAndIdentifyChestColor();
    loader.load("/1cloth-good.glb", function (gltf) {
      const cloth = gltf.scene;
      cloth.scale.setScalar(0.3);
      ...
      const color = new THREE.Color(identifiedColor);
      cloth.traverse(function (el) {
        if (el.isMesh) {
          el.material.color.set(color);
        }
      ...
      push(ref(db, 'clothes'), clothData); // 直接使用 push 方法
